...
function validation(phone) {
    ...
    if (!phone) errors = 'Este campo no puede estar vacío'
        ...
        if ( place && !isPossiblePhoneNumber(phone)) errors = 'Número de teléfono inválido'
    }
    return errors
}
...
function AlertPhoneMissing() {
    ...
    const { alertDialogMessage, closeAlertDialogMessage } = useGetAlertDialogMessage({ type: 'phoneMissing'})
    ...
    async function handleActionButton() {
        ...
        const response = await updateMyAccount({property: 'phoneNumber', value: phone})
        if (response.message) {
            setLoading(false)
    ...
    return (
        ...
        >
            ...
            <DialogContent>
                <DialogContentText>
                    {'Para poder brindarte un servicio de calidad necesitamos que nos indiques tu número telefónico'}
                </DialogContentText>
                <InputPhoneNumber
                    numberPhone={phone}
