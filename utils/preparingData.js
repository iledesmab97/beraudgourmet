...
export function oneUserDataFromFrontToBack(propertyFront) {
    ...
    else if (property === "numberPhone")
        return { ...propertyFront, property: "phoneNumber" };
...
export function descriptionOrder(order, type) {
    ...
    switch (type) {
        case "pizza": {
            ...
            extraIngredients = `${order.mass}${Object.keys(order.extra)
                .map((ingredient) => {
                    return `, ${order.extra[ingredient]}x ${ingredient}`;
                })
                .join("")}`;
            ingredientsOut = order.ingredientsModal
        ...
        case "salad": {
            ...
            extraIngredients = `${Object.keys(order.extra)
                .map((ingredient) => {
                    return `${order.extra[ingredient]}x ${ingredient}`;
                })
                .join(", ")}`;
...
export function extractElements(text, kindProduct) {
    ...
    switch (kindProduct) {
        case "pizza": {
            ...
            extraIngredients = allElements
                .filter((item, index) => index > 1 && !item.includes("~"))
                .map((item) => {
        ...
        case "salad": {
            ...
            extraIngredients = allElements
                .filter((item, index) => index > 0 && !item.includes("~"))
                .map((item) => {
                    const itemDivided = item.split("x ");
                    return {
                        quantity: itemDivided[0],
...
export function descriptionWithoutIngredientsOut(description) {
    ...
    const descriptionWithOutIngredientsOut = allElements
...
export function isSamePizza(pizza1, pizza2) {
    for (let property in pizza1) {
        if (property !== "ingredients" || property !== "price") {
            ...
            if (pizza1[property].length !== pizza2[property].length)
                return false;
            ...
            if (
                Object.keys(pizza1.price).length !==
                Object.keys(pizza2.price).length
            )
                return false;
            for (let [size, masses] of Object.entries(pizza1.price)) {
                if (!(size in pizza2.price)) return false;
                if (
                    Object.keys(masses).length !==
                    Object.keys(pizza2.price[size]).length
                )
                    return false;
...
export function deepUnequal(value1, value2) {
    ...
    const ordersToRemove = value2.itemsxOrder.filter(
        (order) => !currentOrdersListId.includes(order.id)
    );
